...
This script launches Firefox inside the namespace as your user, handling X11/Wayland permissions.
```bash
...
```bash
#!/bin/bash
# Allow root to access the display (required for sudo inside netns)
xhost +si:localuser:root > /dev/null 2>&1

# Replace 'gabx' with your actual username
USER_NAME="gabx"

ip netns exec vpn_client sudo -u $USER_NAME --preserve-env=DISPLAY,XAUTHORITY,WAYLAND_DISPLAY,XDG_RUNTIME_DIR firefox
```
